from Process.Cache import admins
...
from PriyaMusic.config import BOT_USERNAME, GROUP_SUPPORT, UPDATES_CHANNEL, IMG
...
from PriyaMusic.inline import stream_markup
...
async def update_admin(client, message):
    ...
    await message.reply_text(
        f"**ᴀᴅᴍɪɴ ᴄᴀᴄʜᴇ ʀᴇʟᴏᴀᴅᴇᴅ sᴜᴄᴄᴇssꜰᴜʟʟʏ ʙᴀʙʏ​ {message.from_user.mention()}.**"
    )
...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            ...
            await m.reply(f"**➻ sᴛʀᴇᴀᴍ sᴋɪᴩᴩᴇᴅ 🥺\n│ \n└ʙʏ : {m.from_user.mention()} 🥀**\n\n**» ɴᴏ ᴍᴏʀᴇ ǫᴜᴇᴜᴇᴅ ᴛʀᴀᴄᴋs ɪɴ **ʟᴇᴀᴠɪɴɢ ᴠɪᴅᴇᴏᴄʜᴀᴛ**")
        elif op == 2:
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.leave_group_call(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("✅ **This Streaming has Ended**",)
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`",)
    else:
        await query.answer("❌ Nothing is currently Streaming", show_alert=True)
